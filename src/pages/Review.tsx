...
export default function Review() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* Filters */}
      <div className="flex gap-2 flex-wrap border-b border-gray-200 dark:border-gray-800 pb-4 transition-colors">
        {filters.map(f => (
          <button
            key={f.key}
            ...
            className={cn(
              ...
              filter === f.key
                ? f.key === "hard"
                  ? "bg-amber-100 text-amber-800 border border-amber-200 dark:bg-amber-900/40 dark:text-amber-400 dark:border-amber-800/50"
                  : f.key === "due"
                  ? "bg-blue-100 text-blue-800 border border-blue-200 dark:bg-blue-900/40 dark:text-blue-400 dark:border-blue-800/50"
                  : "bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900"
                : "bg-white text-gray-600 hover:bg-gray-50 border border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:border-gray-700"
            )}
          >
            {f.icon}
      ...
      {weakWords.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 border border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-12 text-center max-w-lg mx-auto mt-12 transition-colors">
          <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-500 dark:text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-4 transition-colors">
            <BrainCircuit className="w-8 h-8" />
          </div>
          <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-2">You're all caught up!</h2>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
              ...
              <div className="flex gap-3 flex-wrap md:flex-nowrap w-full md:w-auto">
                <button
                  onClick={handleStudy}
                  className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-blue-600 dark:bg-blue-500 text-white px-5 py-3 rounded-xl font-medium hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors"
                >
                ...
                <button
                  onClick={handleQuiz}
                  className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white px-5 py-3 rounded-xl font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-colors">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400 font-medium border-b border-gray-100 dark:border-gray-700 transition-colors">
                  <tr>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {weakWords.map(word => {
                    ...
                    return (
                      <tr key={word.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">
                          <div className="flex items-center gap-2">
                            {word.isHard && <Star className="w-4 h-4 text-amber-500 fill-current shrink-0" />}
                            <span>{word.term}</span>
                            <span className="text-xs text-gray-400 dark:text-gray-500 font-normal px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded transition-colors">
                              {word.type}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{word.meaning}</td>
                        <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{collection?.name ?? "—"}</td>
                        <td className="px-6 py-4 text-right">
                          {total === 0 ? (
                            <span className="text-gray-400 dark:text-gray-500 text-xs bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full transition-colors">New</span>
                          ) : (
                            <span
                              className={cn(
                                "font-semibold",
